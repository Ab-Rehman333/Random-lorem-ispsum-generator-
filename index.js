...
form.addEventListener("submit", (e) => {
  e.preventDefault();
  let userValue = parseInt(amount.value);
  const getRandomData = async () => {
    let fetchData = await fetch(
      "https://baconipsum.com/api/?type=meat-and-filler"
    );
    let resultData = await fetchData.json();
    let getRandomHardData = Math.floor(Math.random() * text.length)
  if (isNaN(userValue) || userValue < 1 || userValue > 9) {
    result.innerHTML = `<p class="result">${text[getRandomHardData]}</p>`;
  }
   else {
      let sliceTxt = resultData.slice(0, userValue);
      sliceTxt = sliceTxt
        .map((item) => {
          return `<p class="result">${item}</p>`;
        })
        .join("");
      result.innerHTML = sliceTxt;
    }
